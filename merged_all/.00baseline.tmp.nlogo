...
; ---------------------------------------------------Setting up variables and turtles-----------------------------
breed[AGVs AGV]
breed[pods pod]
breed[jobs job]
breed[emptys empty]
...
to setup
  ...
  output-show "------------------------------------"
  generate-order generated-order
  assign-order-to-pod
  place-agv ; OK
  assigning -1
  reset-ticks
...
to setup-globals
  set list-item n-values type-of-item [i -> i] ;setup list of item from 0 to 'type-of-item'
end

to go
  update-queue ;update queue length in pick station
  count-pick-station-idle-time ;counting each of pick station idle time (Time when no queue occured)
  if not any? turtles [stop] ;check if there is a AGV at the world
  ;robot movement
  ask AGVs with [shape = "kiva" or shape = "transparent"][
    let who-id who st (
      ifelse  ;assign instruction who is not hidden
      status = "pick-pod" [
        pick-pod who-id 0
      ]
      status = "bring-to-picking" [
        bring-to-picking who-id
      ]
      status = "queuing" [
        queuing who-id
      ]
  ...
  checking
  detect-traffic ;visualize when the intersection has more than 1 AGV
  ;count output
  if time mod 3600 = 3599 [
    finished-order 3600
  ]
  ;order & replenishment
  if time = next-order-time + 2 [
    generate-order incoming-order
    update-order
    assign-order-to-pod
    update-order
    next-incoming-order
...
to set-layout
  ...
  [
    ...
    ( ifelse
      itemcode = "pod"[
        sprout-pods 1[
          ;Assigning each of every items are assigned to the pods
      ...
      itemcode = "L"[
        set pcolor 9
        sprout 1[
          ifelse pycor = 7 or pycor = 8 or pycor = 38 or pycor = 39 [
            set shape "highway"
          ][
            set shape "direction"
          ]
...
to place-item [podid]
  ask pods with [pod-id = podid][
    set items []
    let n 0
    let sum-item-type range type-of-item
    let shuffle-sum-item-type shuffle sum-item-type
    loop[
      ifelse n < sku-per-pod[
        let one-item 0
        ifelse not empty? list-item[
          set one-item one-of list-item
          set list-item remove one-item list-item
        ][
          set one-item random type-of-item
        ]
        let sku []
;        let item-type itemtype
        ...
        let due 999
        set sku insert-item 0 sku one-item
        ...
        file-type podid file-type "," file-type one-item file-type "," file-type qty file-type "," file-type due file-type "," file-type qty file-type "\n"
        file-close
...
to generate-order [num]
  ...
  loop[
    ifelse n < num[
      ...
      let qty 1
      let due random-poisson 3
      while [due < 1 or due > 5][set due random-poisson 3] ; Random 1~5
      ;export excel
...
to switch-pod [xc yc id]
  ...
  ask pods with [pod-id = transfer-pod-id] [
    set pod-id -1
    set items_ items
    set status_ status
    set replenish_ replenish
  ]
  ask patches with [pxcor = xc and pycor = yc][
    sprout-pods 1[
      set shape "full square"
...
to assign-order-to-pod
  ...
  let result py:runresult "result" ; result with 2 data
  set pod-list2 table:from-list result ;get pod id list
  set pod-list table:keys pod-list2 ; get pod total item quantity
...
to pair-pick-pod [id] ;
  ...
  ifelse length pod-list  > 2 * AGV-number [
    set max-pod-to-assign (2 * AGV-number)
  ] [
    set max-pod-to-assign length pod-list
  ]
  loop[ ;for every AGV
    ...
    ifelse looping-agv < AGV-number[
      loop[ ;for every pod
        ifelse looping-pod < max-pod-to-assign[
          ...
          file-type xcor file-type "," file-type ycor file-type ","
          file-type xstart file-type "," file-type ystart file-type ","
          file-type xend file-type "," file-type yend file-type ","
          file-type xjob file-type "," file-type yjob file-type ","
          file-type u-turn file-type "," file-type AGV-id file-type ","
...
to pair-next [id]
  ...
  [
    ...
    let yjob 0
    let max-pod-to-assign 0
    ifelse length pod-list > 2 [
      set max-pod-to-assign 2
    ][
      set max-pod-to-assign length pod-list
    ]
    set looping-pod 0 ;one AGV with each pod
    loop[  ;for every pod
      ifelse looping-pod < max-pod-to-assign[
        ...
        ending-intersection id destination
        file-open "for pairing.csv"
        file-type xcor file-type "," file-type ycor file-type "," file-type xstart file-type ","
        file-type ystart file-type "," file-type xend file-type "," file-type yend file-type ","
        file-type xjob file-type "," file-type yjob file-type "," file-type u-turn file-type ","
        file-type AGV-id file-type "," file-type destination file-type "\n"
        file-close
        set looping-pod looping-pod + 1
      ]
      [stop]
...
;;to calculate picking service time
to pick-service [AGVid podid]
  ask AGVs with [AGV-id = AGVid]
  [ask pods with [pod-id = podid]
    [set qty-dlv table:get pod-list2 pod-id
      file-open "deliver to pick station.csv"
      file-type pod-id file-type "," file-type qty-dlv file-type "\n"
      file-close
  ]]
end

to assigning [num] ;warning
  print sort pod-list
  if not empty? pod-list[ ;no selected pod left
     let pod-to-assign 0
     let available-AGV 0(
      ifelse
      num = -1 and length pod-list  > AGV-number * 2 [
        set pod-to-assign AGV-number * 2
        set available-AGV AGV-number
      ]
       num = -1 and length pod-list  <= AGV-number * 2 [
        set pod-to-assign length pod-list
        set available-AGV AGV-number
      ]
       num != -1 and length pod-list  > 1 [
        set pod-to-assign 2
        set available-AGV 1
      ]
       num != -1 and length pod-list  <= 1 [
     ...
     ifelse pod-to-assign != 1 [
      ...
      ifelse num = -1[
        ask AGVs [
          ...
          let b 0
          set availability 1
          if a < table:length assignment[  ;prans
            ask pods with [pod-id = table:get assignment a] [
              set status 1
              set b pod-id
            ]
            set destination b
            ...
            ;add pick time
            pick-service AGV-id carrying-pod-id ;tag
            set bring-qty qty-dlv
             if bring-qty > (sku-per-pod * 15)[
              set bring-qty (sku-per-pod * 15)
            ]
            let pick-cycle-time round(random-poisson 6)
            while [pick-cycle-time < 1][
              set pick-cycle-time round(random-poisson 6)
            ]
            set pick-time (bring-qty * pick-cycle-time)
            set start-time time
            set pod-list remove b pod-list]
        ]
      ][
        ask AGVs with [AGV-id = num] [
          let a AGV-id
          let b 0
          set availability 1
          ask pods with [pod-id = table:get assignment a] [
          ...
          set carrying-pod-id destination
          ;add pick time
          pick-service AGV-id carrying-pod-id
          set bring-qty qty-dlv
          if bring-qty > (sku-per-pod * 15)[
            set bring-qty (sku-per-pod * 15)
          ]
      ...
      ask AGVs with [AGV-id = num] [
        ...
        let b 0
        set availability 1
        ask pods with [pod-id = item 0 pod-list] [
          set status 1
          set b pod-id
        ]
        set destination b
...
to assigning-empty
  ...
  ask AGVs with [availability = 0][
    set available-AGV available-AGV + 1
  ]
  while [max-pod-to-assign < (available-AGV * 2)][
    set y-bound (y-bound - pod-size + 1)
    set max-pod-to-assign 0
    ask emptys with [ycor > y-bound and status != 1][
      set max-pod-to-assign max-pod-to-assign + 1]
  ]
  py:set "robotnode" available-AGV
  ...
  ask AGVs with [availability = 0] [
    ...
    set availability 1
    ask emptys with [
      empty-id = table:get assignment a
    ][
      set status 1
      set b empty-id
      set total-empty total-empty - 1
...
to update-queue
  ...
  ;PICKING STATION #1
  ask (
    patch-set patch 4 45
    patch 5 45 patch 6 45
    patch 7 45 patch 7 44
    patch 7 43)[
    ask pick-stations with [xcor = 3 and ycor = 45][
      set queue-length count AGVs-on (
        patch-set patch 4 45
        patch 5 45 patch 6 45
        patch 7 45 patch 7 44
        patch 7 43
  ...
  ;PICKING STATION #2
  ask (patch-set patch 10 45 patch 11 45 patch 12 45 patch 13 45 patch 13 44 patch 13 43)
  [ ask pick-stations with [xcor = 9 and ycor = 45]
    [set queue-length count AGVs-on (patch-set patch 10 45 patch 11 45 patch 12 45 patch 13 45 patch 13 44 patch 13 43)]]

  ;PICKING STATION #3
  ask (patch-set patch 16 45 patch 17 45 patch 18 45 patch 19 45 patch 19 44 patch 19 43)
  [ ask pick-stations with [xcor = 15 and ycor = 45]
    [set queue-length count AGVs-on (patch-set patch 16 45 patch 17 45 patch 18 45 patch 19 45 patch 19 44 patch 19 43)]]

  ;PICKING STATION #4
  ask (patch-set patch 22 45 patch 23 45 patch 24 45 patch 25 45 patch 25 44 patch 25 43)
  [ ask pick-stations with [xcor = 21 and ycor = 45]
    [set queue-length count AGVs-on (patch-set patch 22 45 patch 23 45 patch 24 45 patch 25 45 patch 25 44 patch 25 43)]]

  ;PICKING STATION #5
  ask (patch-set patch 28 45 patch 29 45 patch 30 45 patch 31 45 patch 31 44 patch 31 43)
  [ ask pick-stations with [xcor = 27 and ycor = 45]
    [set queue-length count AGVs-on (patch-set patch 28 45 patch 29 45 patch 30 45 patch 31 45 patch 31 44 patch 31 43)]]

end

to count-pick-station-idle-time
;  set idle-time 0
  ask pick-stations [
    if queue-length = 0 [
...
to pick-pod [b block]
  ...
  ask AGVs with [who = b][
    let n destination
    ask pods with [pod-id = n] [
      set xjob xcor
      set yjob ycor
    ]
    ( ifelse
      path-status = "go-to-aisle" [
        go-to-aisle yjob
      ]
      path-status = "reaching-destination" [
        reaching-destination xjob yjob
      ]
      path-status = "on-the-way" [
        on-the-way
        if path-status = "arrive"[
          set block 1
        ]
      ]
      path-status = "arrive" [
        bring-pod-out
        if block = 1 [
          block-road "?" AGV-id set block 0
        ]
        ask pods with [pod-id = n and shape != "empty space"][
          switch-empty xjob yjob b
          reduce-item-in-pod n
        ]
      ]
    )
  ]
...
to go-to-aisle [yjob]
  ...
  ifelse xcor != xstart[
    ...
    [not-collide])
    ifelse in-one-block? AGV-id destination and ycor = yjob
    [set path-status "reaching-destination"]
    [set path-status "go-to-aisle"]
  ][
    let m 0
    ...
    ifelse in-one-block? AGV-id destination
    [set path-status "reaching-destination"]
    [set path-status "on-the-way"]]
end
...
to on-the-way
  let m 0
  ask patch-here [
    if meaning = "intersection"
    [set m 1]
  ]
  ifelse m = 1[
    (
      ifelse
      xcor = xstart and ycor = ystart and right-way? [
        ifelse can-turn-left? [
      ...
      end-point? and heading = 0 [
        set path-status "reaching-destination"
      ]
      end-point? and heading = 180 [
        set path-status "reaching-destination"
      ]
      turning? [
        ifelse can-turn-left? [
...
to reaching-destination [xj yj]
  let m 0
  ask patch-here [
    if meaning = "podspace" or meaning = "empty-space" [set m 1]
  ]
  (
    ifelse
    ycor = yj and xcor != xj and m = 0[
      (ifelse
        xcor < xj and heading = 0 [
          move-to patch-ahead 0
          rt 90 not-collide
        ]
        xcor < xj and heading = 180 [
          move-to patch-ahead 0
          lt 90 not-collide
        ]
        xcor > xj and heading = 0 [
...
to bring-to-picking [n] ;Full copy Chaterine
  let m 0
  ask patch-here [
    if meaning = "intersection" [set m 1]
  ]
  ask AGV n[
    ifelse m = 1[
        (
          ...
          heading = 90 and ycor = 38 and xcor = 7[
            ask pick-stations with [xcor = 3 and ycor = 45][
              ifelse queue-length > 4[
                ask AGV n [
                  move-to patch-ahead 1 not-collide
                ]
              ][
                ask AGV n [
                  move-to patch-ahead 0
          ...
          heading = 90 and ycor = 38 and xcor = 13[
            ask pick-stations with [xcor = 9 and ycor = 45][
              ifelse queue-length > 4[
                ask AGV n [move-to patch-ahead 1 not-collide]
              ][
                ask AGV n [move-to patch-ahead 0 lt 90 not-collide]
              ]
            ]
          ]

          ;to picking station #3
          heading = 90 and ycor = 38 and xcor = 19[
            ask pick-stations with [xcor = 15 and ycor = 45][
              ifelse queue-length > 4[
                ask AGV n [move-to patch-ahead 1 not-collide]
              ][
          ...
          ;to picking station #4
          heading = 90 and ycor = 38 and xcor = 25[
            ask pick-stations with [xcor = 21 and ycor = 45][
              ifelse queue-length > 4[
                ask AGV n [move-to patch-ahead 1 not-collide]
              ][
          ...
          ;to picking station #5
          heading = 90 and ycor = 38 and xcor = 31[
            ask pick-stations with [xcor = 27 and ycor = 45][
              ifelse queue-length > 4[
                ask AGV n [
                  move-to patch-ahead 0
          ...
          ;to picking station #1 FROM DOWN
          heading = 0 and ycor = 38 and xcor = 7[
            ask pick-stations with [xcor = 3 and ycor = 45][
              ifelse queue-length > 4[
                ask AGV n [move-to patch-ahead 0 rt 90 not-collide]
              ][
                ask AGV n [move-to patch-ahead 0 not-collide]
              ]
            ]
          ]

          ;to picking station #2 FROM DOWN
          heading = 0 and ycor = 38 and xcor = 13[
            ask pick-stations with [xcor = 9 and ycor = 45][
              ifelse queue-length > 4[
                ask AGV n [move-to patch-ahead 0 rt 90 not-collide]
              ][
          ...
          ;to picking station #3 FROM DOWN
          heading = 0 and ycor = 38 and xcor = 19[
            ask pick-stations with [xcor = 15 and ycor = 45][
              ifelse queue-length > 4[
                ask AGV n [move-to patch-ahead 0 rt 90 not-collide]
              ][
          ...
          ;to picking station #4 FROM DOWN
          heading = 0 and ycor = 38 and xcor = 25[
            ask pick-stations with [xcor = 21 and ycor = 45][
              ifelse queue-length > 4[
                ask AGV n [
                  move-to patch-ahead 0 rt 90 not-collide]
              ][
                ask AGV n [move-to patch-ahead 0 not-collide]
              ]
            ]
          ...
          ;to picking station #5 FROM DOWN
          heading = 0 and ycor = 38 and xcor = 31[
            ask pick-stations with [xcor = 27 and ycor = 45][
              ifelse queue-length > 4[
                ask AGV n [
                  move-to patch-ahead 0
                  not-collide move-to patch-ahead 0
                  lt 90 not-collide
                ]
              ][
                ask AGV n [move-to patch-ahead 0 not-collide]
          ...
          ;to picking station #1 FROM RIGHT
          heading = 270 and ycor = 39 and xcor = 7[
            move-to patch-ahead 0 rt 90 not-collide
          ]

          ;to picking station #2 FROM RIGHT
          heading = 270 and ycor = 39 and xcor = 13[
            ask pick-stations with [xcor = 9 and ycor = 45
            ][
              ifelse queue-length > 4[
                ask AGV n [move-to patch-ahead 0 not-collide]
          ...
          ;to picking station #3 FROM RIGHT
          heading = 270 and ycor = 39 and xcor = 19[
            ask pick-stations with [xcor = 15 and ycor = 45][
              ifelse queue-length > 4[
                ask AGV n [move-to patch-ahead 0 not-collide]
              ][
          ...
          ;to picking station #4 FROM RIGHT
          heading = 270 and ycor = 39 and xcor = 25[
            ask pick-stations with [xcor = 21 and ycor = 45][
              ifelse queue-length > 4[
                ask AGV n [move-to patch-ahead 0 not-collide]
              ][
...
to queuing [n]
  ask AGV n[
    ...
    (ifelse
      ...
      ycor = 45 and xcor mod 6 = 4 [
        if status = "queuing"[
          set status "process"
          set count-down pick-time
        ]
      ]
      [not-collide]
    )
...
to not-collide
  ...
  let n 0
  let heading-ahead abs(heading - 180)
  let m 0
  let id AGV-id
  let AGV-ahead-who 0
  carefully[ask AGV-ahead [
    if AGV-id = id [set n 1]
    if hidden? [set m 1]
    if heading = heading-ahead [set heading-ahead "deadlock"]
    set AGV-ahead-who who
    ]
  ][]
  (ifelse
    AGV-ahead = nobody or m = 1 [
      set previous-x xcor
      set previous-y ycor
      ifelse heading = 180 and ycor - 1 <= 8 [
        set shape "transparent"
      ][
        set shape "kiva" set color 9 set size 0.9
      ]
      fd 1
    ]
    n = 1 [
      fd 1 ask AGV-ahead [die]
    ]
    heading-ahead = "deadlock" and n != 1 [
      resolve who resolve AGV-ahead-who
    ]
  )
  if AGV-ahead != nobody or n = 1 [
    if previous-x != xcor or previous-y != ycor[
      ...
      ifelse status = "queuing" [
        set Stop-Que Stop-Que + 1
      ][
        set stopping stopping + 1
      ]
    ]
  ]
  ask AGVs with [shape = "dot"][
    let status-change 0
    let dot-id agv-id
    ask AGVs with [AGV-id = dot-id][
      if path-status != "arrive" [
        set status-change 1
      ]
    ]
...
;        show sort [carrying-pod-id] of AGVs count-robot-cycle-time
        count-robot-cycle-time
        set status "pick-pod"
        pair-next AGV-id
        assigning AGV-id
        block-road xstart AGV-id
      ]
    )
  ]
end

to reduce-item-in-pod [podid]
   py:set "pod_id" pod-id
  (py:run
    "import ManageItemInPod"
    "pod_update = ManageItemInPod.reduceItemInPod(pod_id)")
  let pod_item_now py:runresult "pod_update"
end
...
to block-road [xc id]
  ...
  if xc = "?"[
    set xc xcor
    ask patches with [pxcor = xc - 1 and pycor = yc][
      ifelse meaning = "road-down" or meaning = "road-up" [
        set xc xc - 1
      ][
        set xc xc + 1
      ]
...
to checking
  ask AGVs[
    if xcor = 0 or xcor = 35 [
      ...
      let xc 0
      let yc 0
      ask pods with [pod-id = destination_][
        set destination_ who
        set xc xcor
        set yc ycor
      ]
...
to resolve [who-id]
  ...
  ask AGVs with [who = who-id][
    ...
    ask pods with [pod-id = destination_][
      set destination_ who
      set xc xcor set yc ycor
    ]
    ask patch-here [
      if meaning = "empty-space" or meaning = "podspace"
      [set under-pod 1]
    ]
    if under-pod = 1 [
      ifelse xc = xcor and yc = ycor
      [ht]
      [move-to turtle destination_ set path-status "arrive"]
    ]
  ]
end
...
to-report going-further?
  if heading = 90 and xend < xcor and xcor != xstart
  and xcor != xend and ycor != ystart and ycor != yend [
    report true
  ]
  if heading = 180 and xend > xcor and xcor != xstart
  and xcor != xend and ycor != ystart and ycor != yend [
    report true
  ]
  report false
...
to-report turning?
  ...
  ;direct manhattan
  if xcor = xstart and ycor != ystart and xcor != xend
  and ycor = yend and right-turn? and straight-first != 2 [
    report true
  ]
  if xcor != xstart and ycor = ystart and xcor = xend
  and ycor != yend and right-turn? and straight-first != 2  [
  ...
  if u-turn = 4 and right-turn? and going-further? [
    report true
  ]
