# -*- coding: utf-8 -*-
"""
Created on Tue Jul 14 01:45:24 2020

@author: Chaterine
"""

def calculateTotalItem(total_item):
    import pandas as pd
    import os
    import numpy as np
...
#    
#    selected_pod1 = selected_pod.sort_values(by= ['due_date','qty'])
#    selected_pod1 = selected_pod.reset_index(drop=True)
#    
...
# data_order = pd.read_csv(order_path, names = ["item", "qty", "due_date"])
# data_pod = pd.read_csv(pod_path, names = ["pod_id", "item", "qty", "due_date", "max_qty"])
# data_pod_t = pd.read_csv(pod_path, names = ["pod_id", "item", "qty", "due_date", "max_qty"])

# data_order = data_order.sort_values(by=['due_date','qty'])
# data_order = data_order.reset_index(drop=True)

# data_pod_sorted1 = data_pod_t.sort_values(by=['due_date'])
# data_pod_sorted1 = data_pod_sorted1.reset_index(drop=True)
# data_pod_sorted = data_pod
...
# notAssigned = []
# for j in range (len(data_order.item)):
#     for i in range (len(data_pod_sorted.item)):
#         if data_pod_sorted.item[i] == data_order.item[j] and data_pod.qty[i] >= data_order.qty[j]:
#             data_pod_sorted.qty[i] -= data_order.qty[j]
#             data_pod_sorted.due_date[i] = data_order.due_date[j]
#             selected_pod.loc[len(selected_pod)] = [data_pod_sorted.pod_id[i],data_pod_sorted.due_date[i],data_order.qty[j],time,0]
#             break;
#     if j == (len(data_pod_sorted.item)-1):
#         no_match.append(j)
#         notAssigned.append([data_order.item[j],data_order.qty[j],data_order.due_date[j]])
