def assignOP(time):
    ...
    #delete sort pod based on qty -- error on netlogo
    data_pod_sorted = data_pod.sort_values(by=['due_date'])
    data_pod_sorted = data_pod_sorted.reset_index(drop=True)
    no_match = []
    opsiPod = []
    ...
    # np.savetxt(pod_path, data_pod_sorted, delimiter = ',')  
    return result   
