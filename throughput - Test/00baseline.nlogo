...
to go
  ...
  ifelse replication <= 2 [if time = 21600 [store-result record-stop-and-go record-replication setup ]][ask turtles [die] carefully [file-delete "replication.csv"][]]
...
to place-item [m]
  ...
  [ ;;add how to generate non-duplicate item-type
    py:run "import random"

    set items []

    ;;add how to generate non-duplicate item-type
    py:set "rand" sku-per-pod
    py:set "typeItem" type-of-item ;for range
    set random_number array:from-list py:runresult "random.sample(range(typeItem),rand)"
    ...
    [ ifelse n < sku-per-pod
      [ let sku []
        ;;add how to generate non-duplicate item-type
        let item-type random array:item random_number n
...
to generate-order [num]
  ...
  ;set order-recap csv:from-file "orders.csv"
...
;  py:set "time" time_
;  (py:run
;    "import assignmentOP" ;ini dy masi manggil yg lama lho, bukan yg assignmentOP_BU
;    "result = assignmentOP.assignOP(time)"
;  ;;add to calculate total delivered items per pod
;;    "import totalOP"
;;    "totorder_pod = totalOP.totOP(time)"
;  )
;  set pod-list py:runresult "result"
;  set pod-list remove-duplicates pod-list
;  output-show pod-list
;  py:set "time" time_
;  (py:run
;    "import assignmentOP2" ;ini dy masi manggil yg lama lho, bukan yg assignmentOP_BU
;    "result = assignmentOP2.assignOP(time)"
;  ;;add to calculate total delivered items per pod
;;    "import totalOP"
;;    "totorder_pod = totalOP.totOP(time)"
;  )
;  set pod-list2 py:runresult "result"
;  set pod-list2 remove-duplicates pod-list2
;  output-show pod-list2
;;  ; record total order qty assigned in a pod
;;  set totorder-pod py:runresult "totorder_pod"
;
...
to assign-order-to-pod
  ...
  output-show table_keys
  output-show item 0 table_keys
  let counter 0
  let table_length table:length pod-list2
  set pod-list table_keys
;  set item-dlv table:values pod-list2
;  let n 0
;  loop
;  [ ifelse n < length pod-list
;    [set pod-id item n pod-list
;      set qty-dlv table:get pod-list2 pod-id
;    set n n + 1]
;  [stop]]
;  output-show table_length
;  foreach n-values table_length[i -> i][
;    i -> array:set pod-list 1 1
...
;        "totorder_pod = totalOP.totOP(time)")
;      set totorder-pod py:runresult "totorder_pod"
;      set item-dlv table:from-list totorder-pod
      set qty-dlv table:get pod-list2 pod-id]]
  ...
  if not empty? pod-list;no selected pod left
    [let pod-to-assign 0
     ...
     ifelse pod-to-assign != 1 [
        output-show available-AGV
        output-show pod-to-assign
        ...
        ifelse num = -1[
          ask AGVs [
            ...
            if a < table:length assignment[  ;prans
              ...
              set pick-time (qty-dlv * (round(random-poisson 6)))
              set start-time time
...
;  if status = "at-workstation" [set status "process" set count-down pick-time]
...
to reset-count-down
;  set count-down round(random-poisson 15) ;* totall
   set count-down pick-time
   set label ""
end
