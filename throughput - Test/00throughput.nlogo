...
to go
  ...
  ;order & replenishment
  if time = next-order-time + 2 [generate-order 10 update-order assign-order-to-pod update-order next-incoming-order]
  check-pod

  ;replication (number starts from 0)
  ;;change number of replication and simulation time
  ifelse replication <= 9 [if time = 43200 [store-result record-stop-and-go record-replication setup ]][ask turtles [die] carefully [file-delete "replication.csv"][]]
...
;      [set qty random-poisson 3]
      ...
      file-close
      file-open "order history.csv"
      file-type item-type file-type "," file-type qty file-type "," file-type due file-type "\n"
      file-close
      file-open "likelihood.csv"
      file-type "" file-type "\n"
      file-close

      set n n + 1]
    [
      py:set "total_item" total-item
      (py:run
        "import sumItem"
        "count = sumItem.calculateTotalItem(total_item)")
      set total-item py:runresult "count"
;      output-show (word "total item " total-item)
;      output-show (word "type-of-item " type-of-item)
      py:set "total_item" total-item
      py:set "itemType" type-of-item
      (py:run
        "import LikelihoodRate"
        "LR = LikelihoodRate.CountLikelihoodRate(itemType,total_item)")
      set likelihood-rate py:runresult "LR"
;      output-show (word "LikelihoodRate " likelihood-rate)
      ;set LR-list table:from-list likelihood-rate
      ;set LR-value table:get LR-list pod-id
...
;  output-show (word "table keys " table_keys)
  let counter 0
  let table_length table:length pod-list2
  set pod-list table_keys

;  output-show (word "pod-list " pod-list)
...
to assigning [num]
  if not empty? pod-list;no selected pod left
    [let pod-to-assign 0
     ...
     let assignment 0
      ;;checking
      set check-robotnode available-AGV
      set check-podnode pod-to-assign
      ;;
...
;              output-show table:get assignment a
...
to bring-back [id]
  ...
  [ let n destination
    ...
    ( ifelse
      ...
      path-status = "arrive" [ask emptys with [empty-id = n][switch-pod xjob yjob id] show sort [carrying-pod-id] of AGVs count-robot-cycle-time set status "pick-pod" pair-next AGV-id assigning AGV-id block-road xstart AGV-id])]
end
