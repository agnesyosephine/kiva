...
;  [lead-time-count-down]
  ...
  detect-traffic
  ;count output
  if round timer mod 3600 = 3599 [finished-order 3600]
  ;order & replenishment
  if round timer = next-order-time + 2 [generate-order round(random-poisson num-arrival) update-order assign-order-to-pod update-order next-incoming-order] ;2 -> time to setup and go for the first setting the world
  check-pod

  ;replication (number starts from 0)
  ifelse replication <= (simulation-replication - 1) [if round timer = (simulation-run-hours * 3600) [store-result record-stop-and-go record-replication setup]][ask turtles [die] carefully [file-delete "replication.csv"][]]
...
to set-globals
  ...
  set gap-on-aisle (item 1 gap-on-aisle - item 0 gap-on-aisle)
  set pod-batch-size (gap-on-aisle / 2) - 1
end
...
to set-layout
    ...
    ( ifelse
      ...
      [ sprout-pods 1
        [ set shape "full square"
          set color sky
          set pod-id max [pod-id] of pods + 1
          place-item sku-shuffle]
      ...
      [ sprout-emptys 1
        [ set shape "empty space"
          set color 9
          set empty-id max [empty-id] of emptys + 1]
  ...
  ;storing item data in excel
  file-open "item in pod.csv"
  ask pods [foreach items
    [x -> foreach x
      [ file-type pod-id file-type "," file-type item 0 x file-type "," file-type item 1 x file-type "," file-type item 2 x file-type "," file-type item 3 x file-type "\n" ]
  ]]
  file-close
...
to place-agv
  ask n-of AGV-number patches with [meaning = "podspace"]
  [ sprout-AGVs 1
    [ set AGV-id count AGVs
      set size 0.9
      set color orange
      set availability 1
      set shape "kiva"
      set status "pick-pod"
      set next-empty-id (max [empty-id] of emptys + AGV-id + 1)
      set heading ((random 2) * 180) + 90
;      pair-pick-pod AGV-id
     ]
  ]
end

;------------------------------------------------------------ ORDER & REPLENISHMENT & PICKING STATION ---------------------------------------------------------------------;
...
to next-incoming-order
  ...
  set next-order-time round (timer) + n
end
...
to generate-order [num]
  ...
  [ ifelse n < num
    [ ;random generator
      ...
      let list_ []
      set list_ lput item-type list_
      set list_ lput qty list_
      set list_ lput due list_
      set orders lput list_ orders
...
to update-queue
  ask picking-stations
  [ let destination_ entering-queue
    let xcor_ xcor
    let queue_ count AGVs with [status = "queuing" and xcor <= destination_ and xcor >= xcor_]
    set queue_ queue_ + count AGVs with [status = "bring-to-picking" and destination = destination_]
    set queue-count queue_]
end

;------------------------------------------------------------ OUTPUT ---------------------------------------------------------------------;

to finished-order [cycle]
  py:set "time" round(timer)
  py:set "cycle" cycle
  (py:run
    "import FinishOrder"
...
to count-robot-cycle-time
  set r-cycle-time round(timer) - start-time
...
;  [ ask pods with [pod-id = item x pod-list][if shape = "empty space" [set already-delivered-pod lput pod-id already-delivered-pod]]
;    set x x + 1]
;  set x 0
;  while [x < length already-delivered-pod]
;  [ set pod-list remove item x already-delivered-pod pod-list
;    set x x + 1]
;  selected-pods pod-list
;end

to assign-order-to-pod
  let from_bigger_qty 0
  foreach orders
  [ order_data ->
    while [from_bigger_qty <= max [qty-ordered] of pods]
    [ carefully
      [ ask one-of pods with [qty-ordered = max [qty-ordered] of pods - from_bigger_qty and pod_criteria items order_data != false]
        [ let n 0
          foreach items
          [ items_ ->
            if items_ = pod_criteria items order_data
            [ set items_ replace-item 4 items_ (item 1 order_data)
              set items replace-item n items items_]
            set n n + 1]
          set qty-ordered qty-ordered + item 1 order_data
          set shape "shelf"
          set color 138
          set from_bigger_qty (max [qty-ordered] of pods + 1) ]]
      [ set from_bigger_qty from_bigger_qty + 1]]]

    set pod-list [pod-id] of pods with [shape = "shelf"]
end

to select-pod [order_data]

end

to-report pod_criteria [item_ order_data]
  foreach item_
  [ items_ ->
    if item 0 items_ = item 0 order_data and item 1 items_ >= (item 1 order_data - item 4 items_)
    [report items_]]
  report false
end
...
to ending-intersection-return [id jobloc]
  let xjob 0 let yjob 0 let patch-next-to 0 let roadup_ 0 let xend_ 0
  ask emptys with [empty-id = jobloc] [set xjob xcor set yjob ycor]
  ask AGVs with [AGV-id = id]
  [ set u-turn 0 set straight-first 0 ;return value
    ;determine xend
    ask patches with [pxcor = xjob - 1 and pycor = yjob] [if meaning != "podspace" and meaning != "empty-space" [set patch-next-to "aisle"]]
...
to starting-intersection [id jobloc]
  ...
  [ let n xcor
    ...
    [ (ifelse
      ...
      (ifelse
        xcor mod 2 = 0
        [ask patches with [pxcor = (n + 1) and pycor = storage-upper-ybound] [ifelse meaning = "intersection" [ask AGVs with [AGV-id = id] [set xstart (n + 1)]] [ask AGVs with [AGV-id = id] [set xstart (n - 1)]]]]
        xcor mod 2 = 1
        [ask patches with [pxcor = (n + 2) and pycor = storage-upper-ybound] [ifelse meaning = "intersection"  [ask AGVs with [AGV-id = id] [set xstart (n + 2)]] [ask AGVs with [AGV-id = id] [set xstart (n - 2)]]]])]
...
;  ask patch-here [if meaning = "intersection" [set intersection_ 1]]
  ask AGV n
  [ if ycor >= 38 and ycor < 43 and q-time = 0 and any? AGVs-on patch-ahead 1 [set q-time round(timer)]
    if ycor = 43 and q-time = 0 [set q-time round(timer)]
    (ifelse
  ...
  [ if path-status != "on-the-way" [set availability 0]
    (ifelse
      ycor = 45 and xcor mod 6 = 1 and heading = 0 [move-to patch-ahead 0 lt 90 not-collide
        ...
        ask pods with [pod-id = podid_][set staytime qty-ordered * round(random-gamma service-time-alpha service-time-beta)]
...
to stay
  ...
  if count-down <= 0
    [ if heading != 180 [move-to patch-ahead 0 lt 90]
      ...
      set que-time round(timer) - q-time
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if round(timer) = 1 [plot 0] if round(timer) mod 3600 = 0 [plot finish-order]"
