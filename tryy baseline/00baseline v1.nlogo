...
to setup
  ...
  replication-read
  delete-file
  ;;move py:setup before set-layout;;
...
to go
  ...
  ;;change number of replication and simulation time
...
to delete-file
  ...
  carefully [file-delete "replicate pod data.csv"][]
  if replication = 0
  [ carefully [file-delete "robot cycle time.csv"][]
    ...
    carefully [file-delete "replication.csv"][]
    carefully [file-delete "order history.csv"][]
    carefully [file-delete "likelihood.csv"][]
    carefully [file-delete "deliver to pick station.csv"][]]
  file-open "Assigned_order_to_pod.csv" file-type "" file-close
  file-open "replicate pod data.csv" file-type "" file-close
end
...
to place-item [m]
  ...
  [ set items []
    let n 0
    let try range type-of-item
    let try2 shuffle try

    loop
    [ ifelse n < sku-per-pod
      [ let sku []
        let item-type item 0 try2
        set try2 remove-item 0 try2
...
to next-incoming-order
  let n round(random-exponential 30)
...
;      ; generate random number 1~5 with Poisson distribution for ordered item qty
;      let qty random-poisson 3
;      while [qty < 1 or qty > 3]
;      [set qty random-poisson 3]

      let qty 1
      let due random-poisson 3 ; Random 1~5
      ; generate random number 1~5 with uniform distribution for order due date
      while [due < 1 or due > 5]
      [set due random-poisson 3]
...
to assign-order-to-pod
  ...
  set pod-list2 table:from-list result
  let table_keys table:keys pod-list2 ;list isi 1 data : pod-list
  set pod-list table_keys
      ...
      ifelse num = -1[
        ...
        [
          ask AGVs with [AGV-id = num] [
            let a AGV-id let b 0 set availability 1
            ask pods with [pod-id = table:get assignment a] [set status 1 set b pod-id] set destination b
            starting-intersection AGV-id destination
            ending-intersection AGV-id destination
            set carrying-pod-id destination
...
to update-queue
  ...
  ;PICKING STATION #1
  ask (patch-set patch 4 45 patch 5 45 patch 6 45 patch 7 45 patch 7 44 patch 7 43)
  [ ask pick-stations with [xcor = 3 and ycor = 45]
    [set queue-length count AGVs-on (patch-set patch 4 45 patch 5 45 patch 6 45 patch 7 45 patch 7 44 patch 7 43)]]
...
to pick-pod [b block]
  ...
  ask AGVs with [who = b][
    ...
    ( ifelse
      ...
      path-status = "arrive" [
        bring-pod-out
        if block = 1 [block-road "?" AGV-id set block 0]
...
to bring-to-picking [n]
  ...
  [ ifelse m = 1
    [ (ifelse
      ...
      [ask pick-stations with [xcor = 27 and ycor = 45]
        [ifelse queue-length > 4
          [ask AGV n [move-to patch-ahead 0 lt 90 not-collide move-to patch-ahead 0 lt 90 not-collide]]
          [ask AGV n [move-to patch-ahead 0 lt 90 not-collide]]]]
      ...
      [ask pick-stations with [xcor = 27 and ycor = 45]
        [ifelse queue-length > 4
          [ask AGV n [move-to patch-ahead 0 not-collide move-to patch-ahead 0 lt 90 not-collide]]
          [ask AGV n [move-to patch-ahead 0 not-collide]]]]
