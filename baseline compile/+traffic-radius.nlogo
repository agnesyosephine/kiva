...
to detect-traffic
  ...
  [ let num-AGV 0
    ...
    ask AGVs in-radius traffic-radius [ if towards-traffic? xdirection ydirection xc yc [set num-AGV num-AGV + 1]]
...
to not-collide
  ...
  ask patch-ahead 1 [if pcolor = 15
    [ set traffic_ "yes" let xdirection 0 let ydirection 0 let xc pxcor let yc pycor
      ifelse pxcor mod 2 = 0 [set xdirection "down"][set xdirection "up"]
      ifelse pycor mod 4 = 0 [set ydirection "left"][set ydirection "right"]
      ask AGVs in-radius traffic-radius with [heading = 270 or heading = 90] [ if towards-traffic? xdirection ydirection xc yc [set horizontal horizontal + 1]]
      ask AGVs in-radius traffic-radius with [heading = 270 or heading = 90] [ if towards-traffic? xdirection ydirection xc yc and previously-moving? AGV-id [set vertical vertical + 1]]
  ]]
  (ifelse
