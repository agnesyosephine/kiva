...
to go
  ...
  ;order & replenishment
  if time = next-order-time + 2 [generate-order round(num-arrival) update-order assign-order-to-pod update-order next-incoming-order] ;2 -> time to setup and go for the first setting the world
  check-pod
  ...
  [ if path-status != "on-the-way" [set availability 0]
    (ifelse
      ycor = 45 and xcor mod 6 = 1 and heading = 0 [move-to patch-ahead 0 lt 90 not-collide
        ...
        ask pods with [pod-id = podid_][set staytime qty-ordered * round(service-time-alpha / service-time-beta)]
