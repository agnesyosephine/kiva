...
to setPatchForPod [x y]
  set agv_pod_X insert-item (length agv_pod_X) agv_pod_X x
  set agv_pod_Y insert-item (length agv_pod_Y) agv_pod_Y y
end
  ...
  [
    ...
    ( ifelse
      ...
      [ sprout-pods 1
        ...
        print (word "Patch" pxcor "," pycor)
        setPatchForPod pxcor pycor
      ]
      itemcode = "empty"
      [ sprout-emptys 1
...
to random-place-agv
  ...
  (py:run
    "from randomAGV import random-place-AGV"
    "ret = random-place-AGV(numAGV, numIndex)")
  ...
  [ ifelse n < AGV-number
    [ let idx item n agvloc
      ; output-show (word "" item idx availableXPos ", " item idx availableYPos ", " n)
      ask patches with [pxcor = item idx agv_pod_X and pycor = item idx agv_pod_Y]
      [ sprout-AGVs 1
        [ set AGV-id n
          set size 0.9
          set color 9
...
to place-agv
  ...
  [ ifelse n < AGV-number
    [ let agvcode item n agvloc
     ...
     ; ask patches with [pxcor = Rpxcor and pycor = Rpycor] ;;<--- Added this one
      [ sprout-AGVs 1
        [ set AGV-id n
          set size 0.9
          set color 9
